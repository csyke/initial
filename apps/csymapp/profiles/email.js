...
class Profile extends csystem {
	...
	async postEmailProfile(req, res) {
		let self = this;
		...
		;[err1, care1] = await to(EmailProfile.sendCode(body.email, "Activate", appName, care[0].uid, care[0].FamilyFamilyId, "csystem", "root"));
		if(err1) throw err1;
		return care
	}
	...
	// 		status: 422,
	// 		message: {
	// 			emailid: "Please provide emailid to modify"
	// 		}
	// 	})
	...
	// 	if (care === null) throw ({
	// 		status: 422,
	// 		message: "can't set for another user"
	// 	})
	...
	// 			status: 422,
	// 			message: {
	// 				Permission: "You are not allowed to modify that account"
	// 			}
	// 		})
	...
	// 	[err, care] = await to(Familyfe.EmailProfile.update(data, {
	// 		emailuid: emailid
	// 	}))
	// 	if (err) throw (err);
	...
	// 	let [_err, csyAdmin] = [];
	// 	if (authuid !== uidtoMod) {
	// 		[_err, csyAdmin] = await to(Familyfe.Family.memberHasRoleinFamilyforApp({
	// 			AppName: "csystem"
	// 		}, "root", 1, authuid))
	// 		if (_err) throw ({
	// 			status: 422,
	...
	// 	let data = JSON.parse(JSON.stringify(req.body));
	// 	[err, care] = await to(Familyfe.EmailProfile.delete({
	// 		emailuid: emailid
	// 	}))
	...
	// 		console.log(care)
	// 		if (!(Object.keys(care).length)) throw ({
	// 			status: 422,
	// 			message: {
	// 				code: "You have provided an invalid code"
	// 			}
	// 		})
	// 		if (err) throw (err)
	...
	// 		} else {
	// 			res.json({
	// 				success: "activated. But we don't know where to redirect you to..."
	// 			})
	// 		}
	// 	} else {
	...
	// 			status: 422,
	// 			message: {
	// 				emailid: "Please provide either code or emailid to modify"
	// 			}
	// 		})

	// 		entropy.use(charset8)
	// 		let Code = entropy.string().substring(0, 8);;
	// 		[err, care] = await to(Familyfe.EmailProfile.whichEmailProfile({
	// 			emailuid: emailid
	// 		}))
	// 		if (err) throw err;
	// 		if (care === null) throw ({
	...
	// 		[err, care] = await to(Familyfe.EmailProfile.emailActivation({
	// 			Code,
	// 			EmailprofileEmailuid: euid
	// 		}, Email, activationPath))
	...
	async main(req, res) {
		...
		switch (method) {
			case 'POST':
				;
				[err, care] = await to(self.postEmailProfile(req, res));
				if (err) throw err
				res.json(care)
				break;
				// case 'PATCH':
